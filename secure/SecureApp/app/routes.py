    ...
    if request.method == 'POST':
        ...
        try:
            ...
            if bcrypt.checkpw(password.encode('utf-8'), stored_password.encode('utf-8')):
                ...
                session['user_role'] = user_role  # Add user_role to session
...
def view_messages():
    try:
        ...
        # Pass messages and user to the template
...
def admin_manage_messages():
    try:
        ...
        flash(f'Error: {str(e)}', 'danger')
        return redirect(url_for('dashboard'))  # Redirect to 'dashboard' as a fallback
...
def add_user():
    ...
    # Fetch user data for the current admin
    user_data = supabase.table('users').select('*').eq('email', user_email).execute().data[0]
